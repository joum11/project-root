...
-- users
CREATE TABLE users (user_id SERIAL PRIMARY KEY, email VARCHAR(255) NOT NULL UNIQUE, password_hash VARCHAR(255) NOT NULL, full_name VARCHAR(100) NOT NULL, date_of_birth DATE, gender gender_type, phone VARCHAR(20), role user_role NOT NULL, created_at TIMESTAMP DEFAULT now(), updated_at TIMESTAMP DEFAULT now());
...
-- maintenance logs
CREATE TABLE maintenance_logs (maintenance_id SERIAL PRIMARY KEY, equipment_id INT REFERENCES equipment(equipment_id) ON DELETE CASCADE, room_id INT REFERENCES rooms(room_id) ON DELETE CASCADE, reported_by INT REFERENCES users(user_id), description TEXT NOT NULL, reported_at TIMESTAMP NOT NULL DEFAULT now(), status maintenance_status DEFAULT 'open', resolved_at TIMESTAMP, resolver_id INT REFERENCES users(user_id), CHECK ((status = 'resolved' AND resolved_at IS NOT NULL) OR status <> 'resolved'));

-- maintenance audit
CREATE TABLE maintenance_audit (audit_id SERIAL PRIMARY KEY, maintenance_id INT REFERENCES maintenance_logs(maintenance_id) ON DELETE CASCADE, old_status maintenance_status, new_status maintenance_status, changed_at TIMESTAMP NOT NULL DEFAULT now(), changed_by INT REFERENCES users(user_id));
...
-- health metrics
 CREATE TABLE health_metrics (metric_id SERIAL PRIMARY KEY, member_id INT REFERENCES members(member_id) ON DELETE CASCADE, weight NUMERIC CHECK (weight > 0), heart_rate INT CHECK (heart_rate > 0), recorded_at TIMESTAMP NOT NULL CHECK (recorded_at <= now()));
-- classes
...
-- class sessions
CREATE TABLE class_sessions (session_id SERIAL PRIMARY KEY, class_id INT REFERENCES classes(class_id) ON DELETE CASCADE, session_start TIMESTAMP NOT NULL, session_end TIMESTAMP NOT NULL, room_id INT REFERENCES rooms(room_id) ON DELETE CASCADE, trainer_id INT REFERENCES trainers(trainer_id) ON DELETE CASCADE, capacity INT NOT NULL CHECK (capacity > 0), CHECK (session_start < session_end));
...
CREATE TABLE pt_sessions (pt_session_id SERIAL PRIMARY KEY, member_id INT REFERENCES members(member_id) ON DELETE CASCADE, trainer_id INT REFERENCES trainers(trainer_id) ON DELETE CASCADE, start_time TIMESTAMP NOT NULL, end_time TIMESTAMP NOT NULL, room_id INT REFERENCES rooms(room_id) ON DELETE CASCADE, CHECK (start_time < end_time));
...
CREATE TABLE trainer_availability ( availability_id SERIAL PRIMARY KEY, trainer_id INT REFERENCES trainers(trainer_id) ON DELETE CASCADE, available_start TIMESTAMP NOT NULL, available_end TIMESTAMP NOT NULL, CHECK (available_start < available_end));

-- invoices
CREATE TABLE invoices (invoice_id SERIAL PRIMARY KEY, member_id INT REFERENCES members(member_id) ON DELETE CASCADE, due_date TIMESTAMP NOT NULL, total_amount NUMERIC CHECK (total_amount >= 0), status VARCHAR(20) DEFAULT 'unpaid');
...
-- payments
CREATE TABLE payments (payment_id SERIAL PRIMARY KEY, invoice_id INT REFERENCES invoices(invoice_id) ON DELETE CASCADE, paid_at TIMESTAMP NOT NULL, amount NUMERIC CHECK (amount > 0), method VARCHAR(50) NOT NULL);
